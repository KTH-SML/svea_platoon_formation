...
class svea_platoon:
    def __init__(self):
        ...
        self.ref_path = CubicSpline2D(rxr, ryr)   
        wp_x, wp_y,_,_,_ = calc_spline_course(rxr, ryr, ds=0.2)
        ...
        self.publish_line(wp_x, wp_y, 'path_plan', 'mocap')
        ...
        self.epsilon_w = 0.15        
        self.epsilon = 0.6
        self.desired_velocity = 0.7  #set the desired platoon velocity
        self.desired_dist = 1.5      #set the desired platoon distance
        self.k1 = 5; self.k2 = 5; self.k3 = 0
        #self.k4 = 0.5; self.k5 = 5; self.k6 = 0  
        self.k4 = 1; self.k5 = 3; self.k6 = 0                
        self.error_sum = 0.0; self.max_error_sum = 2; self.K_p = 3.5; self.K_i = 1
        ...
        self.leader_vehicle_old = {"vehicle_name": "svea1", "x":0.0, "y":0.0, "yaw":0.0, "v":0.0, "s_path":0.0, "yaw_path":0.0, "y_path":0.0, "kappa":0.0, "dkappa":0.0, "v_path":self.desired_velocity, "acc":0.0}
        self.leader_vehicle_new = {"vehicle_name": "svea2", "x":0.0, "y":0.0, "yaw":0.0, "v":0.0, "s_path":0.0, "yaw_path":0.0, "y_path":0.0, "kappa":0.0, "dkappa":0.0, "v_path":self.desired_velocity, "acc":0.0}

        self.create_subscription_to_leader_new()
        self.create_subscription_to_leader_input_new()
        self.create_subscription_to_leader_old()
        self.create_subscription_to_leader_input_old()
    ...
    def get_control(self):
        if self.state_counter == 1: #check if state is received from mocap
            ...
            local_state = [x, y, yaw, v, s_path, tild_theta, tild_y, kappa, dkappa, v_path]  #wrap the local state into a FloatArray
            local_state_msg = FloatArray(data=local_state)
            self.state_publisher.publish(local_state_msg)

            if self.leader_vehicle_old["s_path"] - s_path < 0:
                leader_s =  self.ref_path.sx.x[-1] + self.leader_vehicle_old["s_path"] 
            else:
                leader_s =  self.leader_vehicle_old["s_path"]           
            leader_v = self.leader_vehicle_old["v_path"] 
            ds = leader_s - s_path - self.epsilon
            op_old = (leader_v - v_path)/ds
            ...
            if self.switch_flag == 1:
                road_width_left = 0.3
                road_width_right = 0.7
            else:
                road_width_left = 0.7
                road_width_right = 0.3
            if tild_y >= road_width_left - 0.5:
                alpha = -1
            ...
            self.target_steering = np.arctan2(vehicle_kappa*self.L, 1.0) + 0.05 #  kappa = tan(delta)/L
            ...
            #publish debug data
            debug = [tild_e, tild_s, leader["s_path"], s_path, tild_nu, leader["v_path"] , v_path, v, self.target_velocity, tild_theta, tild_y, self.target_steering-0.05, measure_steering, opflow_leader, acc, vehicle_kappa, tild_v]
    ...
    def create_subscription_to_leader_old(self):
        topic_name = "/" + self.leader_vehicle_old["vehicle_name"] + "/local_state"
        rospy.Subscriber(topic_name, FloatArray, self.update_leader_state_old, tcp_nodelay = True, queue_size = 1)

    def create_subscription_to_leader_input_old(self):
        topic_name = "/" + self.leader_vehicle_old["vehicle_name"] + "/control"
        rospy.Subscriber(topic_name, FloatArray, self.update_leader_input_old, tcp_nodelay = True, queue_size = 1)        

    def update_leader_input_old(self, msg):
        self.leader_vehicle_old["acc"] = msg.data[0]

    def update_leader_state_old(self, msg):
        self.leader_vehicle_old["x"] = msg.data[0]
        self.leader_vehicle_old["y"] = msg.data[1]
        self.leader_vehicle_old["yaw"] = msg.data[2]
        self.leader_vehicle_old["v"] = msg.data[3]

        self.leader_vehicle_old["s_path"] = msg.data[4]
        self.leader_vehicle_old["yaw_path"] = msg.data[5]
        self.leader_vehicle_old["y_path"] = msg.data[6]
        self.leader_vehicle_old["kappa"] = msg.data[7]
        self.leader_vehicle_old["dkappa"] = msg.data[8]
        self.leader_vehicle_old["v_path"] = msg.data[9]
