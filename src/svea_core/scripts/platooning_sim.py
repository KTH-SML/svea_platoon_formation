...
class svea_platoon:
    ...
    def get_control(self):
        ...
        #rospy.loginfo('leader_ self.v_path')
        #rospy.loginfo(self.leader_vehicle["v_path"])

        #rospy.loginfo('self.v_path')
        #rospy.loginfo(self.v_path)
        #rospy.loginfo('tild_s')
        #rospy.loginfo(tild_s)
        #rospy.loginfo('tild_e')
        #rospy.loginfo(tild_e)
        #rospy.loginfo('tild_nu')
        #rospy.loginfo(tild_nu)
        #rospy.loginfo('self.opflow_leader')
        #rospy.loginfo(self.opflow_leader)
        ...
        #rospy.loginfo('error1')
        #rospy.loginfo(tild_y*np.sin(tild_theta)/tild_theta)
        #rospy.loginfo('error2')
        #rospy.loginfo(np.sign(v)*tild_theta)
        #rospy.loginfo('kappa')
        #rospy.loginfo(vehicle_kappa)
        ...
        debug = [tild_e, tild_nu, acc, vehicle_acc, self.target_velocity, v, self.leader_vehicle["s_path"] - self.s_path, tild_y*np.sin(tild_theta)/tild_theta, self.target_steering, vehicle_kappa, np.sign(v)*tild_theta]  #wrap the local state into a FloatArray
    ...
    def update_vehicle_state_mocap(self, state):
        ...
        euler = euler_from_quaternion(state.pose.pose.orientation.x, state.pose.pose.orientation.y, state.pose.pose.orientation.z, state.pose.pose.orientation.w)
        self.yaw = euler[2]
        ...
        self.dw_t_sample.append(rospy.get_time())
    ...
    def update_vehicle_state(self, state):
        ...
        if len(self.dw_sample_left) > window_length:
            ...
            # Get the estimate at the current time (last point)
            current_time_index = -1
            self.opflow_wall_left = 0  #optical_flow_l[current_time_index]/self.v
            self.opflow_wall_right =0  #optical_flow_r[current_time_index]/self.v
        ...
        if self.leader_vehicle["vehicle_name"] == "svea0":   
            ...
            ds = leader_s - self.s_path - self.r_g        
            # Parameters for Savitzky-Golay filter
            window_length = 51  # choose an odd number, typically between 5 and 31
            polyorder = 0 # polynomial order, typically between 2 and 5
          
            self.ds_sample.append(ds)
            self.t_sample.append(current_time)
            #rospy.loginfo('ds_sample')
            #rospy.loginfo(self.ds_sample)
            #rospy.loginfo('t_sample')
            #rospy.loginfo(self.t_sample)            
            if len(self.ds_sample) > window_length:
                ds_sample_array = np.array(self.ds_sample)
                # Apply Savitzky-Golay filter to smooth the distance measurements
                ds_smooth = savgol_filter(ds_sample_array, window_length, polyorder)
                ...
                # Compute the logarithm of the smoothed distance
                log_ds_smooth = np.log(ds_smooth)
                log_ds_smooth_filter = savgol_filter(log_ds_smooth, window_length, 1)
                #rospy.loginfo('ds_smooth')
                #rospy.loginfo(list(ds_smooth))
                #rospy.loginfo('log_dsmooth')
                #rospy.loginfo(list(log_ds_smooth))
                # Apply Savitzky-Golay filter to estimate the first derivative of log(distance)
                res = [self.t_sample[i + 1] - self.t_sample[i] for i in range(len(self.t_sample)-1)]
                optical_flow = savgol_filter(log_ds_smooth, window_length, 1, deriv=1, delta=sum(res)/len(res))
                #rospy.loginfo('optical_flow')
                #rospy.loginfo(list(optical_flow))
                # Get the estimate at the current time (last point)
                current_time_index = -1
                self.opflow_leader = optical_flow[current_time_index]
                self.opflow_leader = 0
        ...
        local_state = [self.x, self.y, self.yaw, self.v, self.s_path, self.yaw_path, self.y_path, self.kappa, self.dkappa, self.v_path, self.leader_vehicle["s_path"]]  #wrap the local state into a FloatArray
        local_state_msg = FloatArray(data=local_state)
        self.state_publisher.publish(local_state_msg)
