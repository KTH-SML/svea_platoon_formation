...
class svea_platoon:
    def __init__(self):
        ...
        self.desired_dist = 1.2      #set the desired platoon distance
        self.k1 = 5; self.k2 = 5; self.k3 = 0
        self.k4 = 1; self.k5 = 7; self.k6 = 0                
        ...
        self.leader_vehicle = {"vehicle_name": "svea0", "x":0.0, "y":0.0, "yaw":0.0, "v":0.0, "s_path":0.0, "yaw_path":0.0, "y_path":0.0, "kappa":0.0, "dkappa":0.0, "v_path":self.desired_velocity, "acc":0.0}
    ...
    def get_control(self):
        if self.state_counter == 1: #check if state is received from mocap
            ...
            if self.control_counter == 0: #check if control loop is initiated            
                ...
                self.leader_vehicle["s_path"] = s_path + 0.6
                self.leader_vehicle["x"], self.leader_vehicle["y"] = self.ref_path.calc_position(self.leader_vehicle["s_path"])
                #self.leader_vehicle["yaw"] = self.ref_path.calc_yaw(self.leader_vehicle["s_path"])
                op_real = 0
                ...
                op_real = (self.leader_vehicle["v_path"] - v_path)/ds

                #update leader s_path
                self.leader_vehicle["s_path"] = self.leader_vehicle["s_path"] + self.leader_vehicle["v_path"] * delta_time
                if self.leader_vehicle["s_path"] - s_path>= self.ref_path.sx.x[-1]:
                    self.leader_vehicle["s_path"] = self.leader_vehicle["s_path"] - self.ref_path.sx.x[-1]

                if self.leader_vehicle["s_path"] >  self.ref_path.sx.x[-1]:
                    leader_s = self.leader_vehicle["s_path"] - self.ref_path.sx.x[-1]  
                else:
                    leader_s = self.leader_vehicle["s_path"]
                self.leader_vehicle["x"], self.leader_vehicle["y"] = self.ref_path.calc_position(leader_s)
                #self.leader_vehicle["yaw"] = self.ref_path.calc_yaw(self.leader_vehicle["s_path"])
            #check if leader is circled bt svea        
            if self.leader_vehicle["s_path"] - s_path < 0:
                leader_s =  self.ref_path.sx.x[-1] + self.leader_vehicle["s_path"]
            else:
                leader_s =  self.leader_vehicle["s_path"]  

            self.publish_pose(x, y, yaw, "vehicle_pose", "mocap")
            self.publish_pose(self.leader_vehicle["x"], self.leader_vehicle["y"], self.leader_vehicle["yaw"], "leader_pose", "mocap")

            tild_s = leader_s - s_path
            tild_e = tild_s - self.desired_dist
            tild_nu = self.leader_vehicle["v_path"] - v_path            
            if tild_theta == 0:
                tild_theta = 1e-9
            ...
            acc = self.leader_vehicle["acc"] + self.k4*tild_e + self.k5*tild_nu + self.k6*opflow_leader
            ...
            self.target_steering = np.arctan2(vehicle_kappa*self.L, 1.0) + 0.35 #  kappa = tan(delta)/L
            ...
            #publish debug data
            debug = [tild_e, tild_s, self.leader_vehicle["s_path"], s_path, tild_nu, self.leader_vehicle["v_path"], v_path, v, self.target_velocity, tild_theta, tild_y, self.target_steering-0.35, measure_steering, op_real, acc, vehicle_kappa, tild_v]
